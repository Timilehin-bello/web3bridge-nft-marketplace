...
async function main() {
  ...
  const NFTMarketplace = await ethers.getContractAt(
    "NFTMarketplace",
    config[Number(chainId)].NFTMarketplace.address
  );

  // Fetch Listing Price
  const listingPrice = await NFTMarketplace.getListingPrice();

  console.log(`NFTMarketplace Token fetched: ${NFTMarketplace.target}\n`);
  ...
  const inputPrice = "12";
  const itemPrice = ethers.parseUnits(inputPrice, "ether");
